...
export const loginUser = (email, password) => async dispatch => {
  ...
  myFirebase
    ...
    .then(async user => {
      ...
      dispatch(showSuccessToast("Login realizado com sucesso!"));
    })
    .catch(error => {
      dispatch(loginError("Email e/ou senha incorretos"));
      dispatch(showErrorToast("Email e/ou senha incorretos, tente novamente"));
    });
};
...
export const verifyAuth = () => dispatch => {
  ...
  myFirebase.auth().onAuthStateChanged(async user => {
    if (user !== null) {
      const userSnapshot = await myFirebase
        .firestore()
        .collection("users")
        .doc(user.uid)
        .get();
      ...
      if (userDoc.role !== "owner") {
        ...
        dispatch(
          loginError(
            "O usuário não tem permissão para fazer login nesse sistema"
          )
        );
      } else {
